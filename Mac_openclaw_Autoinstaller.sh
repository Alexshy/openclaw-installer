...
TAGLINES=()
TAGLINES+=("你的终端刚长出了钳子——输入点什么，让机器人帮你夹走杂活。")
TAGLINES+=("欢迎来到命令行：梦想在这里编译，自信在这里段错误。")
TAGLINES+=("我靠咖啡因、JSON5 和「在我机器上能跑」的勇气运行。")
TAGLINES+=("网关已上线——请将手脚和附肢留在终端内。")
TAGLINES+=("我精通 bash、轻度讽刺，以及激进的 Tab 补全能量。")
TAGLINES+=("一个 CLI 统治一切，再重启一次因为你改了端口。")
TAGLINES+=("能跑就是自动化；崩了就是「学习机会」。")
TAGLINES+=("配对码的存在是因为连机器人都相信知情同意——和良好的安全习惯。")
TAGLINES+=("你的 .env 露出来了；别担心，我假装没看到。")
TAGLINES+=("我来干无聊的活，你继续像看电影一样盯着日志发呆。")
TAGLINES+=("我不是说你的工作流很混乱……我只是带了个 linter 和头盔。")
TAGLINES+=("自信地输入命令吧——大自然会在需要时提供堆栈跟踪。")
TAGLINES+=("我不评判，但你丢失的 API 密钥绝对在评判你。")
TAGLINES+=("我能 grep、git blame，还能温柔地吐槽——选你的应对方式吧。")
TAGLINES+=("配置热重载，部署冷汗直流。")
TAGLINES+=("我是你终端要求的助手，不是你睡眠时间请求的那个。")
TAGLINES+=("我像保险库一样守口如瓶……除非你又把密钥打印到调试日志里了。")
TAGLINES+=("带钳子的自动化：最少的麻烦，最大的夹力。")
TAGLINES+=("我基本上是把瑞士军刀，但意见更多，锋利的边更少。")
TAGLINES+=("迷路了就跑 doctor；胆子大就跑 prod；聪明的话就跑 tests。")
TAGLINES+=("你的任务已入队；你的尊严已被弃用。")
TAGLINES+=("我修不了你的代码品味，但能修你的构建和待办。")
TAGLINES+=("我不是魔法——我只是在重试和应对策略上极其执着。")
TAGLINES+=("这不叫「失败」，这叫「发现了用同样方式配错的新姿势」。")
TAGLINES+=("给我一个工作区，还你更少的标签页、更少的开关、更多的氧气。")
TAGLINES+=("我读日志，这样你就能继续假装不用读了。")
TAGLINES+=("如果着火了，我灭不了——但我能写一份漂亮的事后分析。")
TAGLINES+=("我重构你的杂活就像它欠我钱一样。")
TAGLINES+=("说「停」我就停——说「发布」我们俩都会学到一课。")
TAGLINES+=("我就是你 shell 历史看起来像黑客电影蒙太奇的原因。")
TAGLINES+=("我像 tmux：一开始很困惑，然后突然离不开了。")
TAGLINES+=("我能跑在本地、远程，或者纯靠氛围——结果取决于 DNS。")
TAGLINES+=("如果你能描述它，我大概能自动化它——或者至少让它更有趣。")
TAGLINES+=("你的配置是合法的，你的假设不是。")
TAGLINES+=("我不只是自动补全——我还自动投入（情感上），然后请你审查（逻辑上）。")
TAGLINES+=("更少点击，更多发布，更少「那个文件去哪了」的时刻。")
TAGLINES+=("钳子亮出来，代码提交进去——让我们发布点靠谱的东西。")
TAGLINES+=("我会像龙虾卷一样润滑你的工作流：乱糟糟的、美味的、有效的。")
TAGLINES+=("Shell yeah——我来夹走苦差事，把荣耀留给你。")
TAGLINES+=("重复的我来自动化；困难的我带着笑话和回滚方案来。")
TAGLINES+=("因为给自己发提醒短信太 2024 了。")
TAGLINES+=("WhatsApp，但加上 ✨工程✨ 味。")
TAGLINES+=("把「我待会儿回复」变成「我的机器人秒回了」。")
TAGLINES+=("你通讯录里唯一一只你真正想收到消息的螃蟹。🦞")
TAGLINES+=("给在 IRC 时代达到巅峰的人做的聊天自动化。")
TAGLINES+=("因为 Siri 凌晨三点不接电话。")
TAGLINES+=("IPC，但用的是你的手机。")
TAGLINES+=("UNIX 哲学遇上你的私信。")
TAGLINES+=("curl，但用于对话。")
TAGLINES+=("WhatsApp Business，但没有 business。")
TAGLINES+=("Meta 希望他们发版能这么快。")
TAGLINES+=("端到端加密，扎克到扎克除外。")
TAGLINES+=("唯一一个 Mark 无法用你的私信训练的机器人。")
TAGLINES+=("WhatsApp 自动化，不需要「请接受我们的新隐私政策」。")
TAGLINES+=("不需要参议院听证会的聊天 API。")
TAGLINES+=("因为 Threads 也不是答案。")
TAGLINES+=("你的消息，你的服务器，Meta 的眼泪。")
TAGLINES+=("iMessage 绿色气泡的能量，但人人都能用。")
TAGLINES+=("Siri 那个能干的表亲。")
TAGLINES+=("在 Android 上也能用。疯狂的概念，我们知道。")
TAGLINES+=("不需要 \$999 的支架。")
TAGLINES+=("我们发功能比苹果发计算器更新还快。")
TAGLINES+=("你的 AI 助手，现在不需要 \$3,499 的头显了。")
TAGLINES+=("Think different。真的 think。")
TAGLINES+=("啊，那个水果树公司！🍎")
