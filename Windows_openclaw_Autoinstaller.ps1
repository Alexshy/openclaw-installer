...
$tlsUpgraded = $false
try {
    # 尝试 TLS 1.2（数字形式兼容未注册该枚举的旧系统）
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]3072
    $tlsUpgraded = $true
} catch {
    try {
        # 退而求其次：TLS 1.1 + TLS 1.2 组合
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]768 -bor [Net.SecurityProtocolType]3072
        $tlsUpgraded = $true
    } catch {
        try {
            # 最后兜底：使用命名枚举尝试
...
if ($tlsUpgraded) {
    Write-Host "  [OK] TLS 已升级到 1.2 版本" -ForegroundColor Green
} else {
    Write-Host "  [!] TLS 升级未能完全验证，将继续尝试..." -ForegroundColor Yellow
}
...
function Select-Provider {
    ...
    foreach ($p in $script:ProviderList) {
        ...
        Write-Host "  默认模型：$($p.DefaultModel)"
    }

    Write-Host ""
    Write-Host "    提示：国内用户推荐选 1（火山方舟）或 2（阿里百炼），包月订阅更划算。"
    Write-Host ""
...
function Read-ApiKey {
    ...
    Write-Host ""
    Write-Host "    还没有 API Key？前往以下地址获取："
...
function Invoke-GatewayRestart {
    ...
    Write-Host ""
    Write-Host "    小提示：gateway restart 是重启后台 daemon 服务"
    Write-Host "    记得不要额外运行 openclaw gateway run 哦，会造成端口冲突的！"
    Write-Host ""
    try {
        ...
        Write-Host "    可稍后手动运行：openclaw gateway restart"
    }
}
        ...
        switch ($subInput) {
            "1" {
                ...
                try {
                    ...
                    Write-Host "    请手动运行： openclaw configure"
                }
                return
            }
            "2" { return }
...
function Show-WechatChannelMenu {
    ...
    Write-Host "    微信 → 我 → 设置 → 关于微信 → 版本更新"
    Write-Host ""
    Write-Host "  第二步：在微信里启用插件" -ForegroundColor White
    Write-Host "    1. 手机微信 → 「我」→ 「设置」→ 「插件」"
    Write-Host "    2. 找到「微信 ClawBot」，按提示启用/授权"
    Write-Host "    （此步是将微信账号与插件能力绑定）"
    Write-Host ""
    Write-Host "  第三步：点击"继续"，将自动安装微信官方插件" -ForegroundColor White
    Write-Host "    执行命令： npx -y @tencent-weixin/openclaw-weixin-cli@latest install"
    Write-Host "    执行后自动弹出二维码，用需要绑定的微信扫码并点"连接"确认"
    Write-Host ""
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host ""
    Write-Host "    1) " -NoNewline -ForegroundColor Cyan
        ...
        switch ($subInput) {
            "1" {
                ...
                try {
                    ...
                    Write-Host "    请手动运行： npx -y @tencent-weixin/openclaw-weixin-cli@latest install"
                }
                return
            }
            "2" { return }
...
function Show-ChannelsMenu {
    ...
    Write-Host "       进入 Channels 配置总入口，手动选择渠道"
    Write-Host ""
        ...
        switch ($subInput) {
            ...
            "2" {
                ...
                try {
                    ...
                    Write-UIWarn "configure 启动失败：$($_.Exception.Message)"
                    Write-Host "    请手动运行： openclaw configure --section channels"
                }
            }
            "3" {
                Write-Host ""
...
function Show-QQChannelMenu {
    ...
    Write-Host ""
    Write-Host "  接入步骤：" -ForegroundColor White
    Write-Host "    第一步：访问以上官方指南页面，完成 QQ 账号授权"
    Write-Host "    第二步：准备好 QQ 机器人的相关凭据（AppID / Token 等）"
    Write-Host "    第三步：点击继续，进入 OpenClaw Channels 配置页完成绑定"
    Write-Host ""
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host ""
    Write-Host "    1) " -NoNewline -ForegroundColor Cyan
...
function Invoke-SelfCheck {
    ...
    try {
        ...
        Write-Host "    可手动运行： openclaw gateway restart"
    }
    Write-Host ""
...
function Invoke-Dashboard {
    ...
    try {
        ...
        Write-Host "    请手动运行： openclaw dashboard"
    }
    Write-Host ""
}
...
function Invoke-Uninstall {
    ...
    Write-Host "    2. openclaw gateway uninstall"
    Write-Host "    3. openclaw uninstall"
    Write-Host "    4. npm uninstall -g openclaw"
    Write-Host "    5. pnpm remove -g openclaw"
    Write-Host ""
...
function Show-WelcomeMenu {
    ...
    Write-Host "  # 涵盖 OpenClaw环境安装 + OpenClaw最新官方稳定版 + 模型/网关/项目空间全自动部署"
    Write-Host "  # 无后门 | 无病毒 | 全自动 | 全免费 | 零技术门槛"
    Write-Host "  # Created by: Mr_Hou  致力于技术平权降低门槛 让人人都有机会拥抱Ai世界"
    Write-Host "  # Wechat_id：qiyuan_hou，欢迎一起讨论 共同进化！"
    Write-Host "  # **严禁恶意篡改或将本免费脚本商业化售卖**"
    Write-Host "  # ═══════════════════════════════════════════════════════════════════════" -ForegroundColor Gray
    Write-Host "  # 功能菜单:"
    Write-Host ""
    ...
    Write-Host "  （推荐新用户）"
    Write-Host "       自动安装 Node.js / Git / OpenClaw，并配置模型、API Key、网关和项目空间"
    Write-Host "    2) " -NoNewline -ForegroundColor Cyan
    Write-Host "仅自动化安装 OpenClaw" -ForegroundColor White
    Write-Host "       只安装 OpenClaw CLI 运行环境，模型和网关配置稍后可单独完成"
    Write-Host "    3) " -NoNewline -ForegroundColor Cyan
    Write-Host "仅部署 OpenClaw 模型/网关/项目空间" -ForegroundColor White
    Write-Host "       OpenClaw 已安装，仅配置模型提供商、API Key 和工作目录"
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "       支持 DeepSeek / Kimi / 火山方舟 / 阿里百炼 / ChatGPT / Claude 等 9 家提供商"
    Write-Host "    5) " -NoNewline -ForegroundColor Cyan
    Write-Host "添加 Channels（微信 / 飞书 / 企微 / QQ 等即时通讯渠道）" -ForegroundColor White
    Write-Host "       连接即时通讯渠道，让 AI 助手在你的聊天 App 里直接回复消息"
    Write-Host "    6) " -NoNewline -ForegroundColor Cyan
    Write-Host "OpenClaw 自检并尝试修复" -ForegroundColor White
    Write-Host "       自动运行 doctor 诊断 + doctor --fix 修复 + gateway restart 重启网关"
    Write-Host "    7) " -NoNewline -ForegroundColor Cyan
    Write-Host "进入 OpenClaw 配置页面" -ForegroundColor White
    Write-Host "       打开完整的交互式配置向导（模型 / 网关 / 渠道 / 守护进程等）"
    Write-Host "    8) " -NoNewline -ForegroundColor Cyan
    Write-Host "打开 OpenClaw 主页面" -ForegroundColor White
    Write-Host "       启动 OpenClaw Web UI 控制面板，可在浏览器中查看全部功能和对话"
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "       停止全部服务并彻底移除 OpenClaw（操作不可逆，执行前需二次确认）"
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor Gray
    Write-Host ""
...
function Download-FileWithFallback {
    ...
    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
        try {
            Write-Host "    下载中 (方法1/5 尝试 $attempt/$maxAttempts)..."
    ...
    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
        Remove-Item $OutPath -Force -ErrorAction SilentlyContinue
        try {
            Write-Host "    下载中 (方法2/5 尝试 $attempt/$maxAttempts)..."
    ...
    if ($webProxy) {
        for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
            Remove-Item $OutPath -Force -ErrorAction SilentlyContinue
            try {
                Write-Host "    下载中 (方法3/5 尝试 $attempt/$maxAttempts，使用代理)..."
                $wc = [Net.WebClient]::new()
                $wc.Proxy = $webProxy
                $wc.Headers.Add("User-Agent", $ua)
                $wc.DownloadFile($Url, $OutPath)
    ...
    if ($curlExe -and (Test-Path $curlExe)) {
        for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
            Remove-Item $OutPath -Force -ErrorAction SilentlyContinue
            try {
                Write-Host "    下载中 (方法4/5 尝试 $attempt/$maxAttempts)..."
    ...
    if (-not [string]::IsNullOrWhiteSpace($proxy)) {
        for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
            Remove-Item $OutPath -Force -ErrorAction SilentlyContinue
            if ($curlExe -and (Test-Path $curlExe)) {
                Write-Host "    下载中 (方法5/5 尝试 $attempt/$maxAttempts，使用代理)..."
...
function Add-PathToUserEnv {
    ...
    try {
        ...
        Write-Host "[!] 无法写入用户 PATH（权限受限），将在当前会话中临时添加 $Label。" -ForegroundColor Yellow
...
function Ensure-PnpmBinOnPath {
    ...
    try {
        ...
        Write-Host "[!] 无法设置 PNPM_HOME 环境变量（权限受限），将在当前会话中使用临时设置。" -ForegroundColor Yellow
    }
    ...
    foreach ($binDir in $binDirs) {
        ...
        if (-not ($userPath -split ";" | Where-Object { $_ -ieq $binDir })) {
            try {
                ...
                Write-Host "[!] 无法永久添加 $binDir 到用户 PATH（权限受限），已在当前会话中临时添加。" -ForegroundColor Yellow
            }
        }
...
function Invoke-DeployFlow {
    ...
    Write-Host "  $script:UI_LINE" -ForegroundColor Yellow
    Write-Host "    提供商: $($provider.Name)  |  模型: $($provider.DefaultModel)  |  目录: $workDir"
    Write-Host ""
    ...
    while ($true) {
        ...
        switch ($mode) {
            "full" {
                ...
                Write-Host "  $script:UI_LINE" -ForegroundColor Yellow
                Write-Host "    提供商: $($provider.Name)  |  模型: $($provider.DefaultModel)"
