...
function Write-UISection {
    ...
    Write-Host ""
    if ($Step) {
        Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor DarkGray
...
function Invoke-ConfigureModel {
    ...
    Write-Host "       配置指南： " -NoNewline -ForegroundColor DarkGray
    Write-Host "https://www.volcengine.com/docs/82379/2183190?lang=zh" -ForegroundColor Cyan
    Write-Host "       API Key 查看： " -NoNewline -ForegroundColor DarkGray
...
function Show-ChannelsMenu {
    ...
    Write-Host "       参考指南： " -NoNewline -ForegroundColor DarkGray
...
function Show-WelcomeMenu {
    ...
    Write-Host "  # ═══════════════════════════════════════════════════════════════════════" -ForegroundColor DarkGray
    Write-Host "  # 功能菜单:" -ForegroundColor DarkGray
    Write-Host ""

    # ── 安装部署 OpenClaw 篇 ──
    Write-Host "  ┌─ 《安装与部署 OpenClaw 篇》 ────────────────────────────────────────────┤" -ForegroundColor DarkGray
    Write-Host "    1) " -NoNewline -ForegroundColor Cyan
    Write-Host "安装 OpenClaw 并自动化部署" -NoNewline -ForegroundColor White
    Write-Host "  （推荐新用户）" -ForegroundColor DarkGray
    Write-Host "       自动安装 Node.js / Git / OpenClaw，并配置模型、API Key、网关和项目空间" -ForegroundColor DarkGray
    Write-Host "    2) " -NoNewline -ForegroundColor Cyan
    ...
    Write-Host "       OpenClaw 已安装，仅配置模型提供商、API Key 和工作目录" -ForegroundColor DarkGray
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor DarkGray
    Write-Host ""

    # ── 使用 OpenClaw 篇 ──
    Write-Host "  ┌─ 《使用 OpenClaw 篇》（需已安装 OpenClaw）─────────────────────────────┤" -ForegroundColor DarkGray
    Write-Host "    4) " -NoNewline -ForegroundColor Cyan
    Write-Host "更换 OpenClaw 模型（配置 AI 模型提供商 / API Key）" -ForegroundColor White
    Write-Host "       支持 DeepSeek / Kimi / 火山方舟 / 阿里百炼 / ChatGPT / Claude 等 9 家提供商" -ForegroundColor DarkGray
    Write-Host "    5) " -NoNewline -ForegroundColor Cyan
    Write-Host "添加 Channels（微信 / 飞书 / 企微 / QQ 等即时通讯渠道）" -ForegroundColor White
    Write-Host "       连接即时通讯渠道，让 AI 助手在你的聊天 App 里直接回复消息" -ForegroundColor DarkGray
    Write-Host "    6) " -NoNewline -ForegroundColor Cyan
    Write-Host "OpenClaw 自检并尝试修复" -ForegroundColor White
    Write-Host "       自动运行 doctor 诊断 + doctor --fix 修复 + gateway restart 重启网关" -ForegroundColor DarkGray
    ...
    Write-Host "       启动 OpenClaw Web UI 控制面板，可在浏览器中查看全部功能和对话" -ForegroundColor DarkGray
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor DarkGray
    Write-Host ""

    # ── 卸载 OpenClaw 篇 ──
    Write-Host "  ┌─ 《卸载 OpenClaw 篇》 ──────────────────────────────────────────────────┤" -ForegroundColor DarkGray
    Write-Host "    9) " -NoNewline -ForegroundColor Cyan
    Write-Host "完全卸载 OpenClaw" -ForegroundColor White
    Write-Host "       停止全部服务并彻底移除 OpenClaw（操作不可逆，执行前需二次确认）" -ForegroundColor DarkGray
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor DarkGray
    Write-Host ""
