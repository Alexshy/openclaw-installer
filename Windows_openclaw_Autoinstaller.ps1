...
function Write-UILine { param([string]$Char = "-"); Write-Host ("$Char" * 60) -ForegroundColor Gray }
function Write-UISection {
    param([string]$Title, [string]$Step = "")
    Write-Host ""
    if ($Step) {
        Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
        Write-Host "  $script:UI_ICON_ARROW $Title  " -NoNewline -ForegroundColor White
        Write-Host "($Step)" -ForegroundColor Gray
        Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    } else {
        Write-Host "  $script:UI_LINE" -ForegroundColor Gray
...
function Select-Provider {
    ...
    foreach ($p in $script:ProviderList) {
        ...
        Write-Host "  默认模型：$($p.DefaultModel)" -ForegroundColor Gray
    }

    Write-Host ""
    Write-Host "    提示：国内用户推荐选 1（火山方舟）或 2（阿里百炼），包月订阅更划算。" -ForegroundColor Gray
    Write-Host ""
    ...
    while ($true) {
        ...
        if ([int]::TryParse($userInput, [ref]$num) -and $num -ge 1 -and $num -le 9) {
            ...
            Write-Host "    默认模型：" -NoNewline -ForegroundColor Gray
...
function Read-ApiKey {
    ...
    Write-Host ""
    Write-Host "    还没有 API Key？前往以下地址获取：" -ForegroundColor Gray
    Write-Host "    $($Provider.ApiKeyUrl)" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "    环境变量名：" -NoNewline -ForegroundColor Gray
...
function Read-Workspace {
    ...
    Write-Host "    后续工作文件均存放于此目录。" -ForegroundColor Gray
    Write-Host ""
    Write-Host "    默认路径：" -NoNewline -ForegroundColor Gray
    Write-Host $defaultPath -ForegroundColor Yellow
    Write-Host ""
    Write-Host "    直接按 Enter 使用默认路径，或输入自定义路径：" -ForegroundColor Gray
...
function Invoke-Deployment {
    ...
    Write-Host ""
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host "  部署计划" -ForegroundColor White
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host "    模型提供商    " -NoNewline -ForegroundColor Gray
    Write-Host "$($Provider.Name) ($($Provider.NameEn))" -ForegroundColor White
    Write-Host "    认证方式 ID   " -NoNewline -ForegroundColor Gray
    Write-Host $Provider.AuthChoice -ForegroundColor White
    Write-Host "    默认模型      " -NoNewline -ForegroundColor Gray
    Write-Host $Provider.DefaultModel -ForegroundColor White
    Write-Host "    工作目录      " -NoNewline -ForegroundColor Gray
    Write-Host $WorkDir -ForegroundColor White
    Write-Host "    安装 Daemon   " -NoNewline -ForegroundColor Gray
    Write-Host "是（后台服务自启动）" -ForegroundColor White
    Write-Host "    跳过通道配置  " -NoNewline -ForegroundColor Gray
    Write-Host "是（稍后可通过 openclaw channels add 添加）" -ForegroundColor White
    Write-Host "    跳过技能配置  " -NoNewline -ForegroundColor Gray
    Write-Host "是（稍后可通过 openclaw skills 配置）" -ForegroundColor White
    Write-Host "    跳过搜索配置  " -NoNewline -ForegroundColor Gray
    ...
    if ($onboardExit -eq 0) {
        ...
        Write-Host ""
        Write-Host "    已完成以下配置：" -ForegroundColor Gray
        ...
        Write-Host ""
        Write-Host "    你可以稍后手动运行以下命令重试：" -ForegroundColor Gray
...
function Invoke-GatewayRestart {
    ...
    Write-Host ""
    Write-Host "    小提示：gateway restart 是重启后台 daemon 服务" -ForegroundColor Gray
    Write-Host "    记得不要额外运行 openclaw gateway run 哦，会造成端口冲突的！" -ForegroundColor Gray
    Write-Host ""
    try {
        ...
        Write-Host "    可稍后手动运行：openclaw gateway restart" -ForegroundColor Gray
    }
}
...
function Show-DeploySummary {
    ...
    Write-Host "    Gateway 端口  " -NoNewline -ForegroundColor Gray
    ...
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host "    自检修复      " -NoNewline -ForegroundColor Gray
    Write-Host "openclaw doctor --fix" -ForegroundColor Cyan
    Write-Host "    重启服务      " -NoNewline -ForegroundColor Gray
    Write-Host "openclaw gateway restart" -ForegroundColor Cyan
    Write-Host "    打开面板      " -NoNewline -ForegroundColor Gray
    Write-Host "openclaw dashboard" -ForegroundColor Cyan
    Write-Host "    重新引导      " -NoNewline -ForegroundColor Gray
    Write-Host "openclaw onboard --install-daemon" -ForegroundColor Cyan
    Write-Host "    安全审计      " -NoNewline -ForegroundColor Gray
    Write-Host "openclaw security audit --deep" -ForegroundColor Cyan
    Write-Host "    添加通道      " -NoNewline -ForegroundColor Gray
    Write-Host "openclaw channels add" -ForegroundColor Cyan
    Write-Host "    配置技能      " -NoNewline -ForegroundColor Gray
...
function Invoke-ConfigureModel {
    Write-Host ""
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host "  🦞 更换 OpenClaw 模型  " -NoNewline -ForegroundColor White
    Write-Host "配置向导" -ForegroundColor Gray
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host "  模型资源与配置参考" -ForegroundColor White
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host ""
    Write-Host "    🔥 火山方舟 Coding Plan（火山引擎 包月订阅）" -ForegroundColor Yellow
    Write-Host "       配置指南： " -NoNewline -ForegroundColor Gray
    Write-Host "https://www.volcengine.com/docs/82379/2183190?lang=zh" -ForegroundColor Cyan
    Write-Host "       API Key 查看： " -NoNewline -ForegroundColor Gray
    Write-Host "https://console.volcengine.com/ark/region:ark+cn-beijing/apiKey" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "    ☁️  阿里百炼 Coding Plan（平行引擎 包月订阅）" -ForegroundColor Yellow
    Write-Host "       配置指南： " -NoNewline -ForegroundColor Gray
    Write-Host "https://bailian.console.aliyun.com/cn-beijing/?tab=doc#/doc/?type=model&url=3023085" -ForegroundColor Cyan
    Write-Host "       API Key 查看： " -NoNewline -ForegroundColor Gray
    Write-Host "https://bailian.console.aliyun.com/cn-beijing?tab=coding-plan#/efm/coding-plan-detail" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "    🟦 腾讯 Coding Plan（混元大模型 包月订阅）" -ForegroundColor Yellow
    Write-Host "       配置指南： " -NoNewline -ForegroundColor Gray
    Write-Host "https://cloud.tencent.com/document/product/1772/128949" -ForegroundColor Cyan
    Write-Host "       API Key 查看： " -NoNewline -ForegroundColor Gray
    Write-Host "https://hunyuan.cloud.tencent.com/#/app/subscription" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
        ...
        switch ($subInput) {
            "1" {
                ...
                try {
                    ...
                    Write-Host "    请手动运行： openclaw configure" -ForegroundColor Gray
                }
                return
            }
            "2" { return }
...
function Show-WechatChannelMenu {
    Write-Host ""
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host "  🦞 连接微信  " -NoNewline -ForegroundColor White
    Write-Host "Wechat Channel" -ForegroundColor Gray
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    ...
    Write-Host "    微信 → 我 → 设置 → 关于微信 → 版本更新" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  第二步：在微信里启用插件" -ForegroundColor White
    Write-Host "    1. 手机微信 → 「我」→ 「设置」→ 「插件」" -ForegroundColor Gray
    Write-Host "    2. 找到「微信 ClawBot」，按提示启用/授权" -ForegroundColor Gray
    Write-Host "    （此步是将微信账号与插件能力绑定）" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  第三步：点击"继续"，将自动安装微信官方插件" -ForegroundColor White
    Write-Host "    执行命令： npx -y @tencent-weixin/openclaw-weixin-cli@latest install" -ForegroundColor Gray
    Write-Host "    执行后自动弹出二维码，用需要绑定的微信扫码并点"连接"确认" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host ""
    Write-Host "    1) " -NoNewline -ForegroundColor Cyan
        ...
        switch ($subInput) {
            "1" {
                ...
                try {
                    ...
                    Write-Host "    请手动运行： npx -y @tencent-weixin/openclaw-weixin-cli@latest install" -ForegroundColor Gray
                }
                return
            }
            "2" { return }
...
function Show-ChannelsMenu {
    Write-Host ""
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host "  🦞 添加 Channels  " -NoNewline -ForegroundColor White
    Write-Host "连接即时通讯渠道" -ForegroundColor Gray
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "       参考指南： " -NoNewline -ForegroundColor Gray
    Write-Host "https://www.feishu.cn/content/article/7613711414611463386" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "    3) " -NoNewline -ForegroundColor Cyan
    Write-Host "连接企微" -ForegroundColor White
    Write-Host "       参考指南： " -NoNewline -ForegroundColor Gray
    Write-Host "https://open.work.weixin.qq.com/help2/pc/cat?doc_id=21657" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "       进入 Channels 配置总入口，手动选择渠道" -ForegroundColor Gray
    Write-Host ""
        ...
        switch ($subInput) {
            ...
            "2" {
                ...
                try {
                    ...
                    Write-UIWarn "configure 启动失败：$($_.Exception.Message)"
                    Write-Host "    请手动运行： openclaw configure --section channels" -ForegroundColor Gray
                }
            }
            "3" {
                Write-Host ""
...
function Show-QQChannelMenu {
    Write-Host ""
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host "  🦞 连接 QQ  " -NoNewline -ForegroundColor White
    Write-Host "三步完成 QQ 渠道接入" -ForegroundColor Gray
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "  接入步骤：" -ForegroundColor White
    Write-Host "    第一步：访问以上官方指南页面，完成 QQ 账号授权" -ForegroundColor Gray
    Write-Host "    第二步：准备好 QQ 机器人的相关凭据（AppID / Token 等）" -ForegroundColor Gray
    Write-Host "    第三步：点击继续，进入 OpenClaw Channels 配置页完成绑定" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host ""
    Write-Host "    1) " -NoNewline -ForegroundColor Cyan
...
function Invoke-SelfCheck {
    ...
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  小提示：请将完整自检结果复制粘贴给 豆包 / 千问 / DeepSeek 帮你分析" -ForegroundColor Cyan
    Write-Host ""
    ...
    try {
        ...
        Write-Host "    可手动运行： openclaw gateway restart" -ForegroundColor Gray
    }
    Write-Host ""
...
function Invoke-Dashboard {
    ...
    try {
        ...
        Write-Host "    请手动运行： openclaw dashboard" -ForegroundColor Gray
    }
    Write-Host ""
}
...
function Invoke-Uninstall {
    ...
    Write-Host "    1. openclaw gateway stop" -ForegroundColor Gray
    Write-Host "    2. openclaw gateway uninstall" -ForegroundColor Gray
    Write-Host "    3. openclaw uninstall" -ForegroundColor Gray
    Write-Host "    4. npm uninstall -g openclaw" -ForegroundColor Gray
    Write-Host "    5. pnpm remove -g openclaw" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host "  🦞 开始执行卸载流程..." -ForegroundColor White
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host ""
    ...
    # 卸载结果汇总
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host "  🦞 卸载流程完成 — 执行结果汇总" -ForegroundColor White
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host ""
...
function Show-WelcomeMenu {
    ...
    Write-Host "  # OpenClaw 全自动安装部署脚本 - Windows 版" -ForegroundColor White
    Write-Host "  # 涵盖 OpenClaw环境安装 + OpenClaw最新官方稳定版 + 模型/网关/项目空间全自动部署" -ForegroundColor Gray
    Write-Host "  # 无后门 | 无病毒 | 全自动 | 全免费 | 零技术门槛" -ForegroundColor Gray
    Write-Host "  # Created by: Mr_Hou  致力于技术平权降低门槛 让人人都有机会拥抱Ai世界" -ForegroundColor Gray
    Write-Host "  # Wechat_id：qiyuan_hou，欢迎一起讨论 共同进化！" -ForegroundColor Gray
    Write-Host "  # **严禁恶意篡改或将本免费脚本商业化售卖**" -ForegroundColor Gray
    Write-Host "  # ═══════════════════════════════════════════════════════════════════════" -ForegroundColor Gray
    Write-Host "  # 功能菜单:" -ForegroundColor Gray
    Write-Host ""

    # ── 安装部署 OpenClaw 篇 ──
    Write-Host "  ┌─ 《安装与部署 OpenClaw 篇》 ────────────────────────────────────────────┤" -ForegroundColor Gray
    Write-Host "    1) " -NoNewline -ForegroundColor Cyan
    Write-Host "安装 OpenClaw 并自动化部署" -NoNewline -ForegroundColor White
    Write-Host "  （推荐新用户）" -ForegroundColor Gray
    Write-Host "       自动安装 Node.js / Git / OpenClaw，并配置模型、API Key、网关和项目空间" -ForegroundColor Gray
    Write-Host "    2) " -NoNewline -ForegroundColor Cyan
    Write-Host "仅自动化安装 OpenClaw" -ForegroundColor White
    Write-Host "       只安装 OpenClaw CLI 运行环境，模型和网关配置稍后可单独完成" -ForegroundColor Gray
    Write-Host "    3) " -NoNewline -ForegroundColor Cyan
    Write-Host "仅部署 OpenClaw 模型/网关/项目空间" -ForegroundColor White
    Write-Host "       OpenClaw 已安装，仅配置模型提供商、API Key 和工作目录" -ForegroundColor Gray
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor Gray
    Write-Host ""

    # ── 使用 OpenClaw 篇 ──
    Write-Host "  ┌─ 《使用 OpenClaw 篇》（需已安装 OpenClaw）─────────────────────────────┤" -ForegroundColor Gray
    Write-Host "    4) " -NoNewline -ForegroundColor Cyan
    Write-Host "更换 OpenClaw 模型（配置 AI 模型提供商 / API Key）" -ForegroundColor White
    Write-Host "       支持 DeepSeek / Kimi / 火山方舟 / 阿里百炼 / ChatGPT / Claude 等 9 家提供商" -ForegroundColor Gray
    Write-Host "    5) " -NoNewline -ForegroundColor Cyan
    Write-Host "添加 Channels（微信 / 飞书 / 企微 / QQ 等即时通讯渠道）" -ForegroundColor White
    Write-Host "       连接即时通讯渠道，让 AI 助手在你的聊天 App 里直接回复消息" -ForegroundColor Gray
    Write-Host "    6) " -NoNewline -ForegroundColor Cyan
    Write-Host "OpenClaw 自检并尝试修复" -ForegroundColor White
    Write-Host "       自动运行 doctor 诊断 + doctor --fix 修复 + gateway restart 重启网关" -ForegroundColor Gray
    Write-Host "    7) " -NoNewline -ForegroundColor Cyan
    Write-Host "进入 OpenClaw 配置页面" -ForegroundColor White
    Write-Host "       打开完整的交互式配置向导（模型 / 网关 / 渠道 / 守护进程等）" -ForegroundColor Gray
    Write-Host "    8) " -NoNewline -ForegroundColor Cyan
    Write-Host "打开 OpenClaw 主页面" -ForegroundColor White
    Write-Host "       启动 OpenClaw Web UI 控制面板，可在浏览器中查看全部功能和对话" -ForegroundColor Gray
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor Gray
    Write-Host ""

    # ── 卸载 OpenClaw 篇 ──
    Write-Host "  ┌─ 《卸载 OpenClaw 篇》 ──────────────────────────────────────────────────┤" -ForegroundColor Gray
    Write-Host "    9) " -NoNewline -ForegroundColor Cyan
    Write-Host "完全卸载 OpenClaw" -ForegroundColor White
    Write-Host "       停止全部服务并彻底移除 OpenClaw（操作不可逆，执行前需二次确认）" -ForegroundColor Gray
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor Gray
    Write-Host ""
...
function Choose-InstallMethodInteractive {
    ...
    Write-Host "    1) 更新源码目录（git）并使用" -ForegroundColor Gray
    Write-Host "    2) 通过 npm 全局安装" -ForegroundColor Gray
    Write-Host "    3) 通过 pnpm 全局安装" -ForegroundColor Gray
...
function Choose-RegistryInteractive {
    ...
    Write-Host "    1) npm 官方    2) 淘宝 (taobao)   3) 腾讯云 (tencent)" -ForegroundColor Gray
    Write-Host "    4) 清华 (tsinghua)  5) 中科大 (ustc)  6) 网易 (163)" -ForegroundColor Gray
    Write-Host "    7) 华为云 (huawei)  8) 跳过（自动测速）" -ForegroundColor Gray
...
function Download-FileWithFallback {
    ...
    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
        try {
            Write-Host "    下载中 (方法1/5 尝试 $attempt/$maxAttempts)..." -ForegroundColor Gray
    ...
    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
        Remove-Item $OutPath -Force -ErrorAction SilentlyContinue
        try {
            Write-Host "    下载中 (方法2/5 尝试 $attempt/$maxAttempts)..." -ForegroundColor Gray
    ...
    if ($webProxy) {
        for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
            Remove-Item $OutPath -Force -ErrorAction SilentlyContinue
            try {
                Write-Host "    下载中 (方法3/5 尝试 $attempt/$maxAttempts，使用代理)..." -ForegroundColor Gray
                $wc = [Net.WebClient]::new()
                $wc.Proxy = $webProxy
                $wc.Headers.Add("User-Agent", $ua)
                $wc.DownloadFile($Url, $OutPath)
    ...
    if ($curlExe -and (Test-Path $curlExe)) {
        for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
            Remove-Item $OutPath -Force -ErrorAction SilentlyContinue
            try {
                Write-Host "    下载中 (方法4/5 尝试 $attempt/$maxAttempts)..." -ForegroundColor Gray
    ...
    if (-not [string]::IsNullOrWhiteSpace($proxy)) {
        for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
            Remove-Item $OutPath -Force -ErrorAction SilentlyContinue
            if ($curlExe -and (Test-Path $curlExe)) {
                Write-Host "    下载中 (方法5/5 尝试 $attempt/$maxAttempts，使用代理)..." -ForegroundColor Gray
...
function Show-InstallPlan {
    ...
    Write-Host "    安装方式    " -NoNewline -ForegroundColor Gray
    ...
    if ($InstallMethod -eq "npm" -or $InstallMethod -eq "pnpm") {
        ...
        $distTag = "latest"
        Write-Host "    安装包      " -NoNewline -ForegroundColor Gray
        Write-Host "$pkg@$distTag" -ForegroundColor White
    } else {
        Write-Host "    Git 目录    " -NoNewline -ForegroundColor Gray
        Write-Host $GitDir -ForegroundColor White
        Write-Host "    Git 更新    " -NoNewline -ForegroundColor Gray
        Write-Host $(if ($NoGitUpdate) { "已禁用" } else { "已启用" }) -ForegroundColor White
    }
    if ($Registry) {
        $regUrl = Get-RegistryUrl -Key $Registry
        if ($regUrl) {
            Write-Host "    npm 源      " -NoNewline -ForegroundColor Gray
    ...
    if ($GitHubProxy) {
        Write-Host "    GitHub 代理  " -NoNewline -ForegroundColor Gray
        Write-Host $GitHubProxy -ForegroundColor White
    }
    if ($DetectedCheckout) {
        Write-Host "    检测仓库    " -NoNewline -ForegroundColor Gray
        Write-Host $DetectedCheckout -ForegroundColor White
    }
    if ($DryRun) {
        Write-Host "    模拟运行    " -NoNewline -ForegroundColor Gray
...
function Show-SystemInfo {
    ...
    Write-Host "    系统      " -NoNewline -ForegroundColor Gray
    ...
    Write-Host "    架构      " -NoNewline -ForegroundColor Gray
    ...
    if ($null -ne $memGB) {
        Write-Host "    内存      " -NoNewline -ForegroundColor Gray
...
function Invoke-InstallFlow {
    ...
    Write-Host ""
    if ($InstallMethod -eq "git") {
        Write-Host "  源码目录  " -NoNewline -ForegroundColor Gray
        Write-Host $finalGitDir -ForegroundColor Cyan
    }
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host "  相关链接" -ForegroundColor White
    Write-Host "  $script:UI_LINE" -ForegroundColor Gray
    Write-Host "    官方文档      " -NoNewline -ForegroundColor Gray
    Write-Host "https://docs.openclaw.ai/zh-CN" -ForegroundColor Cyan
    Write-Host "    OpenClawCN    " -NoNewline -ForegroundColor Gray
...
function Invoke-DeployFlow {
    Write-Host ""
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host "  🦞 OpenClaw 部署配置器  " -NoNewline -ForegroundColor White
    Write-Host "基于官方源码引导流程" -ForegroundColor Gray
    Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "  $script:UI_LINE" -ForegroundColor Yellow
    Write-Host "    提供商: $($provider.Name)  |  模型: $($provider.DefaultModel)  |  目录: $workDir" -ForegroundColor Gray
    Write-Host ""
    ...
    while ($true) {
        ...
        switch ($mode) {
            "full" {
                ...
                Write-Host "  $script:UI_LINE" -ForegroundColor Yellow
                Write-Host "    提供商: $($provider.Name)  |  模型: $($provider.DefaultModel)" -ForegroundColor Gray
                ...
                Write-Host ""
                Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
                Write-Host "  🦞 OpenClaw 安装器（安装 + 自动部署）  " -NoNewline -ForegroundColor White
                Write-Host "Windows 版" -ForegroundColor Gray
                Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
                Write-Host ""
            ...
            "install" {
                ...
                Write-Host ""
                Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
                Write-Host "  🦞 OpenClaw 安装器（仅安装）  " -NoNewline -ForegroundColor White
                Write-Host "Windows 版" -ForegroundColor Gray
                Write-Host "  $script:UI_LINE_WIDE" -ForegroundColor Gray
                Write-Host ""
